...
library(circlize)
celltype_composition_mtx = celltype_composition_df %>% 
    select(sample_id, celltype, celltype_ratio) %>%
    pivot_wider(names_from = celltype, values_from = celltype_ratio) %>%
    column_to_rownames('sample_id') %>%
    as.matrix

celltype_composition_meta = celltype_composition_df %>% 
    select(sample_id, FINAL_CALL) %>% distinct %>% 
    column_to_rownames('sample_id') %>%
    .[rownames(celltype_composition_mtx), ,drop=F]

gradient_colors = c('#6B9AC2','#FFC3B7', '#EF5A50') %>% setNames(seq(0,1, length.out = 3))
pdf(str_glue('{analysis_root}/out/cell_type_composition_heatmap.pdf'),w = 10, h = 10)
Heatmap(celltype_composition_mtx %>% scale %>% t, 
    name = 'Scaled Cell type composition', 
    col = colorRamp2(seq(0,1, length.out = 3), gradient_colors),
    column_split = celltype_composition_meta$FINAL_CALL,
    ) 
dev.off()
